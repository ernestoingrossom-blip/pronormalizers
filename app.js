...
function islandLink(item,i){return `<a class="island-card ${positions[i]}" href="#island/${item.id}" aria-label="${escapeAttr(item.number+'. '+item.title)}"><span class="island-index">${item.number}</span><span class="island-tag">${item.tag}</span><strong>${item.title}</strong><small>${normalizeLatexText(item.subtitle)}</small><span class="island-arrow" aria-hidden="true">↗</span></a>`}
